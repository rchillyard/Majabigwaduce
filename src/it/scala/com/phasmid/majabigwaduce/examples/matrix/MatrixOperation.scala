...
object MatrixOperation:
  ...
  def dot[X: Numeric](as: Seq[X], bs: Seq[X]): Try[X] =
    ...
    then Try(((as zip bs) map product).sum)
  ...
  /**
   * Returns a failure due to incompatible lengths of the two sequences in the given tuple.
   *
   * @param t a tuple containing two sequences, representing the input data to be checked for length compatibility.
   * @tparam X the element type of the sequences.
   * @return a Failure wrapping an IncompatibleLengthsException indicating the sizes of the two sequences.
   */
  def getException[X](t: (Seq[X], Seq[X])): Try[X] =
